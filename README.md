...
+ 다운받은 패키지 데이터를 캐시에 저장함으로 중복 설치를 진행하지 않아 npm 대비 패키지 설치 속도가 빠릅니다. 또한 yarn.lock은 모든 기기에 동일한 패키지를 설치하기에 버전 차이로 인한 버그를 방지해줍니다. 따라서 "속도", "안정성" 측면에서 npm대비 우수하여 yarn을 채택하였습니다.
